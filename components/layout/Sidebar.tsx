    ...
    <aside className="sidebar">
      <div className="px-5 py-4 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900 leading-tight">ARM Stats</p>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
        {NAV_ITEMS.map((section) => {
          const visibleItems = section.items.filter(
            (item) => !(item.adminOnly && user.rol !== "admin_global")
          );
      ...
      <div className="px-3 py-3 border-t border-gray-100">
        <div className="flex items-center gap-3 px-2 py-2 rounded-lg">
          <div
            ...
            style={{ backgroundColor: "var(--arm-purple-light)", color: "var(--arm-purple)" }}
          >
          ...
          <form action={signOut}>
            <button type="submit" className="text-gray-400 hover:text-gray-600 transition-colors p-1" title="Cerrar sesión">
              <LogOut size={14} />
            </button>
          </form>
        </div>
