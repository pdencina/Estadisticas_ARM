      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            Resumen consolidado por semana — todos los campus
          </p>
        </div>
        <GenerarInformeBtn />
      </div>
...
function InformeCard({ informe }: { informe: InformeSemanal }) {
  const inicio = format(parseISO(informe.semana_inicio), "dd MMM", { locale: es });
    ...
    <div className="card overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 flex flex-wrap items-center gap-3">
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-400 mt-0.5">{inicio} – {fin}</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-gray-100">
        {[
          { label: "Total asistentes", value: informe.total_general },
          { label: "En auditorio",     value: informe.total_auditorio },
          { label: "Aceptaron a Jesús", value: informe.total_paj, accent: true },
          { label: "Contador de almas", value: informe.contador_almas, purple: true },
        ].map((stat) => (
      ...
      {/* Campus breakdown */}
      {informe.datos_por_campus?.length > 0 && (
          ...
          <table className="data-table text-xs">
            ...
            <tbody>
              {informe.datos_por_campus.map((c) => (
                <tr key={c.campus_id}>
                  <td className="font-medium">{c.campus_nombre}</td>
                  ...
                  <td className="text-right tabular-nums font-medium" style={{ color: "var(--arm-teal)" }}>
                    {c.total_paj}
                  </td>
                  <td className={`text-right tabular-nums text-xs font-medium
                    ${c.diferencia_general > 0 ? "text-teal-500" :
                      c.diferencia_general < 0 ? "text-red-500" : "text-gray-400"}`}>
                    {c.diferencia_general > 0 ? "↑" : c.diferencia_general < 0 ? "↓" : "="}{" "}
                    {Math.abs(c.diferencia_general)}
                  </td>
                </tr>
              ))}
            </tbody>
